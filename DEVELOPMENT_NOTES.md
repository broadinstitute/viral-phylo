...
1. check out the git code repository for this module on your local host machine (`git clone https://github.com/broadinstitute/viral-phylo.git`) and edit with your favorite code/text editor
...
1. install this module's dependencies inside the container (within container: `/opt/viral-ngs/viral-phylo/docker/install-dev-layer.sh`)
1. (optional) snapshot this docker image locally if you want to continue using it and skip the above steps in the future (`docker commit <image hash> local/viral-phylo-dev`)
1. test code and execution interactively within the container (`cd /opt/viral-ngs/viral-phylo; pytest -rsxX -n auto test/unit`)
